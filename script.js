...
function OnChoose(event){
    ...
    if (cnt == 3){
		...
		var highscore = 0;
        var thing;		
        const show = document.querySelector("#finalize");
		for (let i = 0; i < result.length ; i ++) {
			score[result[i]] ++;
			chosen[i] = result[i];
		}
        ...
        document.getElementById("resultTitle").innerHTML = "You got: " + thing.title;
        document.getElementById("resultDetails").innerHTML = thing.contents;
        highscore = 0;
    }
}
